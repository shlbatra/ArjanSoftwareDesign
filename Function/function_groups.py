# Grouping functions 
# class group of functions with certain context using self
# grouping of functions can be any kind of iterable ex. list, tuple, set, dict
# datstructure with group of functions - class
# property -> order class and compute price -> class with property 
# class - structured data using dataclasses with couple of methods
# inheritance for abstraction or polymorphism
# class should be data or 
# behaviour focussed (not put instance variable - grp of functions - instead use module)
...
PAYMENT_POST_PROCESSORS_LIST = [   # list of functions
...
PAYMENT_POST_PROCESSORS_TUPLE = (  # immutable so cannot add extra processors after tuple is created
    send_email_to_support,  # better testing of code and simplifying code
    store_analytics,
    update_stock_levels,
    subscribe_to_newsletter,
)
...
def handle_payment_post_processors(
    ...
    for post_processor in post_processors:  # call each fn
